 ...
 *     - mode=json      → redirect to /wp-json/psl/v1/cert-json?course_id=...
 *     - mode=json_all  → redirect to /wp-json/psl/v1/cert-json-all?token=...
 */
final class Psl_Magic_Login {
    ...
    public static function init() {
        ...
        add_action('wp_ajax_psl_magic_create_all', [__CLASS__, 'ajax_create_request_all']); // NEW
        ...
        fetch(STATUS_URL, {
            ...
            .then(j => {
                ...
                if (j.status === 'approved') {
                    ...
                    setTimeout(() => {
                        window.location.href = CONSUME_URL;
                    }, 200);
                    return;
                }
                if (j.status === 'expired') {
...
<?php
        ...
        if ( get_query_var('psl_after_login') || isset($_GET['psl_after_login']) ) {
            ...
            $uid       = get_current_user_id(); // ensure available here
            ...
            } elseif ($mode === 'json_all') {
                $dest = add_query_arg([
                    'token' => $token,
                ], home_url('/wp-json/psl/v1/cert-json-all'));
            } else {
                // mode=pdf
    ...
    /** AJAX: create request (Device 1) — single course (legacy) */
    public static function ajax_create_request() {
        ...
        // Parse course_id from provided certificate URL
        $course_id = 0;
    ...
    /** AJAX: create request for ALL courses (Device 1) — NEW */
    public static function ajax_create_request_all() {
        if (!is_user_logged_in()) wp_send_json_error('not_logged_in');
        $user_id = get_current_user_id();
        $mode    = 'json_all';
